    ...
    <div class="row">
        <div class="col-md-12">
            <div class="pull-left status">
                <a <?= strpos(Yii::$app->request->url, '?verified') !== false ? 'class="active"' : '' ?> href="<?= Url::to(['/xcoin/funding-overview', 'verified' => Funding::FUNDING_REVIEWED]) ?>"><?= Yii::t('XcoinModule.funding', 'Verified') ?></a>
                <a <?= strpos(Yii::$app->request->url, '?verified') === false ? 'class="active"' : '' ?> href="<?= Url::to(['/xcoin/funding-overview']) ?>"><?= Yii::t('XcoinModule.funding', 'Under review') ?></a>
            </div>
